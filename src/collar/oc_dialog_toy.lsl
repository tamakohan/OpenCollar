...
// modified a bit to work in a non-OpenCollar object that has its own scripts in the *same* link
...
integer LINK_CLIENT = LINK_THIS; // was LINK_ALL_OTHERS
...
CleanList() {
    ...
    for (n = iLength - g_iStrideLength; n >= 0; n -= g_iStrideLength) {
        ...
        if (iNow > iDieTime) {
            ...
            llMessageLinked(LINK_CLIENT, DIALOG_TIMEOUT, "", kID);
            RemoveMenuStride(n);
        }
    }
        ...
        for (; i<num_detected;i++){
            ...
            if (bReturnFirstMatch || (sFind != "")) { //if we're supposed to be finding the first match,
                if (llSubStringIndex(llToLower(llDetectedName(i)),llToLower(sFind))==0 
                    ...
                    llMessageLinked(LINK_CLIENT, DIALOG_RESPONSE, llList2String(lParams,0) + "|" + (string)llDetectedKey(i)+ "|0|" + llList2String(lParams,5), (key)llList2String(lSensorInfo,3));
        ...
        //give a dialog with the options on the button labels
            ...
            if (iNum == CMD_OWNER || iNum == CMD_TRUSTED) {
                ...
                if (llGetSubString(sCmd, 0, 3) == "url:") {
                    ...
                    llMessageLinked(LINK_CLIENT, iNum, "menu", kID);
                    ...
                    llMessageLinked(LINK_CLIENT, DIALOG_RESPONSE, llList2String(lParams, 0) + "|" + llList2String(lParams, 1) + "|" + llList2String(lParams, 2), llList2String(lParams, 3));
                } else if (llGetSubString(sCmd, 0, 7) == "timeout:")
                    llMessageLinked(LINK_CLIENT, DIALOG_TIMEOUT, "", llGetSubString(sCmd, 8, -1));
            }
        }
            ...
            else if (sStr == "LINK_REQUEST") llMessageLinked(LINK_CLIENT,LINK_UPDATE,"LINK_DIALOG","");
    ...
    listen(integer iChan, string sName, key kID, string sMessage) {
        ...
        if (~iMenuIndex) {
            ...
            if (sMessage == MORE) Dialog(kID, sPrompt, items, ubuttons, ++iPage, kMenuID, iDigits, iAuth,sExtraInfo);
                ...
                llMessageLinked(LINK_CLIENT, DIALOG_RESPONSE, (string)kAv + "|" + sAnswer + "|" + (string)iPage + "|" + (string)iAuth, kMenuID);
            }
        }
    }
